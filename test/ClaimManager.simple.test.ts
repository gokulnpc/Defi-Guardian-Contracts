...
describe("ClaimManager - Simple Tests", function () {
  ...
  describe("Deployment and Basic Setup", function () {
    it("Should deploy with correct initial parameters", async function () {
      ...
      expect(await claimManager.router()).to.equal(
        await mockRouter.getAddress()
      );
    ...
    it("Should allow owner to update voting parameters", async function () {
      ...
      expect(await claimManager.votingPeriodSeconds()).to.equal(
        newVotingPeriod
      );
    ...
    it("Should reject invalid parameters", async function () {
      // Zero voting period
      await expect(
        claimManager.setParams(0, 2000)
      ).to.be.revertedWithCustomError(claimManager, "BadParams");

      // Quorum > 100%
      await expect(
        claimManager.setParams(300, 10001)
      ).to.be.revertedWithCustomError(claimManager, "BadParams");
    });
  ...
  describe("Allowlist Management", function () {
    it("Should manage destination chain allowlist", async function () {
      ...
      expect(await claimManager.allowlistedDestChains(chainSelector)).to.be
        .false;
    ...
    it("Should manage gas limits by chain", async function () {
      ...
      expect(await claimManager.gasLimitByChain(chainSelector)).to.equal(
        gasLimit
      );
    });
    ...
    it("Should only allow owner to manage allowlists", async function () {
      await expect(
        claimManager.connect(user).allowlistDestChain(12345n, true)
      ).to.be.revertedWithCustomError(
        claimManager,
        "OwnableUnauthorizedAccount"
      );
      ...
      await expect(
        claimManager
          .connect(user)
          .allowlistReceiver(ethers.randomBytes(32), true)
      ).to.be.revertedWithCustomError(
        claimManager,
        "OwnableUnauthorizedAccount"
      );
      ...
      await expect(
        claimManager.connect(user).setGasLimit(12345n, 200000n)
      ).to.be.revertedWithCustomError(
        claimManager,
        "OwnableUnauthorizedAccount"
      );
  ...
  describe("Contract Updates", function () {
    ...
    it("Should allow owner to update voting mirror", async function () {
      ...
      await expect(
        claimManager.setVotingMirror(await newVotingMirror.getAddress())
      )
        .to.emit(claimManager, "VotingMirrorUpdated")
        .withArgs(await newVotingMirror.getAddress());

      expect(await claimManager.votingMirror()).to.equal(
        await newVotingMirror.getAddress()
      );
    });
    ...
    it("Should allow owner to update policy manager", async function () {
      ...
      await expect(
        claimManager.setPolicyManager(await newPolicyManager.getAddress())
      )
        .to.emit(claimManager, "PolicyManagerUpdated")
        .withArgs(await newPolicyManager.getAddress());

      expect(await claimManager.policyManager()).to.equal(
        await newPolicyManager.getAddress()
      );
    });
    ...
    it("Should only allow owner to update contracts", async function () {
      ...
      await expect(
        claimManager.connect(user).setRouter(await newRouter.getAddress())
      ).to.be.revertedWithCustomError(
        claimManager,
        "OwnableUnauthorizedAccount"
      );
  ...
  describe("MockCCIPRouter", function () {
    ...
    it("Should handle CCIP send with correct fee", async function () {
      ...
      await expect(
        mockRouter.ccipSend(12345n, mockMessage, { value: fee })
    ...
    it("Should revert CCIP send with insufficient fee", async function () {
      ...
      await expect(
        mockRouter.ccipSend(12345n, mockMessage, { value: fee / 2n })
